...
const DetailItem = ({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) => (
    <div>
        ...
        <dd className="mt-1 text-md text-gray-900 ml-8 break-words">{children}</dd>
    </div>
);
      ...
      <div className="max-w-4xl mx-auto bg-white p-6 sm:p-8 rounded-xl shadow-lg">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div className="w-full md:w-auto">
                <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 break-words">{buyer.fullName}</h1>
                <div className="flex flex-col sm:flex-row sm:items-center text-md text-gray-600 mt-2 gap-2 sm:gap-4">
                    <span className="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1.5 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                        <span className="break-all">{buyer.email || 'No email'}</span>
                    </span>
                    <span className="flex items-center">
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full md:w-auto">
                 <Link href="/buyers" className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 whitespace-nowrap">
                    Go Back
                </Link>
                {canEdit && (
                    <Link href={`/buyers/${buyer.id}/edit`} className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 whitespace-nowrap">
                        Edit Buyer
                    </Link>
                )}
            </div>
             ...
             <section>
                 <dl className="grid grid-cols-1 gap-y-6">
                    ...
                    <DetailItem label="Notes" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>}>
                        <p className="whitespace-pre-wrap break-words">{buyer.notes || 'No notes provided.'}</p>
                    </DetailItem>
                 </dl>
             </section>
        </div>
